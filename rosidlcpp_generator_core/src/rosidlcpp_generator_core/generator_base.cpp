...
bool is_primitive(const nlohmann::json& type) {
  ...
  return PRIMITIVE_TYPES.find(type["name"].get<std::string>()) != PRIMITIVE_TYPES.end();
}

bool is_float(const nlohmann::json& type) {
  return type["name"] == "float" || type["name"] == "double" || type["name"] == "long double";
}
...
bool is_unsigned_integer(const nlohmann::json& type) {
  const std::set<std::string_view> UNSIGNED_TYPES = {"uint8", "uint16", "uint32", "uint64"};
  return UNSIGNED_TYPES.find(type["name"].get<std::string>()) != UNSIGNED_TYPES.end();
}

bool is_signed_integer(const nlohmann::json& type) {
  const std::set<std::string_view> SIGNED_TYPES = {"int8", "int16", "int32", "int64"};
  return SIGNED_TYPES.find(type["name"].get<std::string>()) != SIGNED_TYPES.end();
}
